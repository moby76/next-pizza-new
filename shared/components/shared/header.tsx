...
export const Header = ({ hasSearch = true, hasCart = true, className }: HeaderProps) => {//по умолчанию значение для параметра hssearch = true
    ...
    //стейт для закрития/открвытия модального окна авторизации
    ...
    useEffect(() => {
        ...
        let toastMessage = ''
        //подтверждение на успешный заказ
        if (searchParams.has('paid')) {//если в поисковой строке будет параметр paid
            ...
            //подтверждение на успешную регистрацию
            if (searchParams.has('verified')) {//если в поисковой строке будет параметр paid
                //
